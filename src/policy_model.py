...
class PolicyModel:
    ...
    def __init__(self):
        ...
        self._fc_W: Float32[Tensor, "hidden_dim flat_size"] = self._init_weight(
            self._hidden_dim,
            flat_size,
        )
        self._fc_b: Float32[Tensor, "hidden_dim"] = self._init_bias(
            self._hidden_dim,
        )

        self._gru_in_W: Float32[Tensor, "combined_hidden_dim hidden_dim"] = (
            self._init_weight(3 * self._hidden_dim, self._hidden_dim)
        )
        self._gru_in_b: Float32[Tensor, "combined_hidden_dim"] = self._init_bias(
            3 * self._hidden_dim,
        )
        ...
        self._gru_hidden_b: Float32[Tensor, "combined_hidden_dim"] = self._init_bias(
            3 * self._hidden_dim,
        )

        self._policy_W: Float32[Tensor, "out_heads hidden_dim"] = self._init_weight(
            2,
            self._hidden_dim,
        )
        self._policy_b: Float32[Tensor, "out_heads"] = self._init_bias(2)

    # Ensure helpers don't require self argument with staticmethod decorator.
    @staticmethod
    def _init_weight(*dim: int) -> Tensor:
        """Create weights ready for autograd using 0.02 stability scaling."""
        return torch.randn(*dim, requires_grad=True) * 0.02

    @staticmethod
    def _init_bias(*dim: int) -> Tensor:
        """Create biases ready for autograd using 0.02 stability scaling."""
    ...
    def _extract_patches(
        ...
        # out_H and out_W in this case are the patch coordinate (kernel position), kernel_size represents the features inside each patch.
    ...
    def _maxpool(
        ...
        # Return the maximum over the kernel axes, which are indices inside each receptive field.
    ...
    def forward(
        ...
        # Compress C, H, and W into one axis for the fc.
